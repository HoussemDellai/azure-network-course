...
#         az network bastion ssh -n ${azurerm_bastion_host.bastion.name} -g ${azurerm_resource_group.rg-hub.name} --target-resource-id ${module.vm-spoke1.vm_id} --auth-type password --username ${var.vm_admin_username}
        
#         az network bastion ssh -n ${azurerm_bastion_host.bastion.name} -g ${azurerm_resource_group.rg-hub.name} --target-resource-id ${module.vm-spoke2.vm_id} --auth-type password --username ${var.vm_admin_username}
