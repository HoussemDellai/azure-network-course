...
resource "azurerm_route" "route-to-nva-hub-spoke1" {
  name                   = "route-to-nva-hub-spoke1"
  resource_group_name    = azurerm_resource_group.rg-hub.name
  route_table_name       = azurerm_route_table.route-table-to-nva-hub.name
  address_prefix         = azurerm_subnet.snet-spoke1-vm.address_prefixes[0] # "0.0.0.0/0"
  next_hop_type          = "VirtualAppliance"
  next_hop_in_ip_address = azurerm_linux_virtual_machine.vm-nva-paloalto.private_ip_address
}

resource "azurerm_route" "route-to-nva-hub-spoke2" {
  name                   = "route-to-nva-hub-spoke2"
  resource_group_name    = azurerm_resource_group.rg-hub.name
  route_table_name       = azurerm_route_table.route-table-to-nva-hub.name
  address_prefix         = azurerm_subnet.snet-spoke2-vm.address_prefixes[0] # "0.0.0.0/0"
  next_hop_type          = "VirtualAppliance"
  next_hop_in_ip_address = azurerm_linux_virtual_machine.vm-nva-paloalto.private_ip_address
}
...
# resource "azurerm_route" "route-to-nva-hub-vmb" {
#   name                   = "route-to-nva-hub-vmb"
#   resource_group_name    = azurerm_resource_group.rg-hub.name
#   route_table_name       = azurerm_route_table.route-table-to-nva-hub.name
#   address_prefix         = azurerm_subnet.snet-spoke2-vm.address_prefixes[0] # "10.2.0.0/16" #  # "${azurerm_private_endpoint.pe-aca-environment.private_service_connection.0.private_ip_address}/32"
#   next_hop_type          = "VirtualAppliance"
#   next_hop_in_ip_address = azurerm_firewall.firewall.ip_configuration.0.private_ip_address
# }
