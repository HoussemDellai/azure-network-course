resource "azurerm_resource_group_policy_assignment" "policy-mssql" {
  name                 = "policy-mssql"
  ...
  parameters = jsonencode({
    privateDnsZoneId = {
      value = azurerm_private_dns_zone.dns-zone-mssql.id
    }
    effect = {
...
resource "azurerm_resource_group_policy_remediation" "remediation-mssql" {
...
resource "azurerm_role_assignment" "role-network-contributor-policy-mssql" {
  scope                = azurerm_private_dns_zone.dns-zone-mssql.id
