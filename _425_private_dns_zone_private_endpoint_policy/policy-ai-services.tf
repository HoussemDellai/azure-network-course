...
resource "azurerm_resource_group_policy_assignment" "policy-ai-services" {
  ...
  parameters = jsonencode({
    privateDnsZoneId = {
      value = azurerm_private_dns_zone.private-dns-zone-ai-services["privatelink.cognitiveservices.azure.com"].id
    }
    effect = {
...
resource "azurerm_resource_group_policy_remediation" "remediation-ai-services" {
  name                 = "remediation-ai-services"
  resource_group_id    = azurerm_resource_group.rg.id
  policy_assignment_id = azurerm_resource_group_policy_assignment.policy-ai-services.id
}

resource "azurerm_role_assignment" "role-network-contributor-policy-ai-services" {
  scope                = azurerm_resource_group.rg.id
  role_definition_name = "Network Contributor"
  principal_id         = azurerm_resource_group_policy_assignment.policy-ai-services.identity.0.principal_id
}
