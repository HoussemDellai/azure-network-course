output "commands" {
  value = <<-EOT
        ...
        az network bastion ssh -n ${module.bastion.bastion_name} -g ${azurerm_resource_group.rg-hub.name} --target-resource-id ${module.vm-hub-nva.vm_id} --auth-type password --username ${var.vm_admin_username}
        ...
        curl ${module.vm-spoke1.vm_private_ip}

        curl ${module.vm-spoke2.vm_private_ip}
